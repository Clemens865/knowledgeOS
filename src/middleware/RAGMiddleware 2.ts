...
export class RAGMiddleware {
  ...
  async processQuery(
    ...
    try {
      ...
      try {
        const fallbackContext = await this.performFallbackSearch(searchQuery);
   ...
   */
  private createNaturalContext(ragContext: RAGContext, config: RAGConfiguration): string {
   ...
   */
  private createMinimalContext(ragContext: RAGContext, config: RAGConfiguration): string {
   ...
   */
  private updateStats(ragContext: RAGContext, totalTime: number): void {
